...
public class TransactionMapperTests {
  ...
  void transactionResponseToTransactionModelParseTest() {
    ...
    Assertions.assertEquals(
        transactionResponse.transactionAt(), transactionModel.getTransactionAt());
  ...
  void transactionToTransactionResponseParseTest(
    ...
    lenient()
        .when(
            awesomeExchangePricesClient.dailyPrice(
    ...
    for (var tag : transactionResponse.tags()) {
      var tagModel =
          transaction.getTags().stream().filter(t -> tag.id().equals(t.getId())).findFirst();
