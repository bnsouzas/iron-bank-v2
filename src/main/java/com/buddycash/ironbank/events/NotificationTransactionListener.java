...
@Service
public class NotificationTransactionListener implements MessageListener<String, TransactionDataEvent> {
    @Override
    ...
    public void onMessage(ConsumerRecord<String, TransactionDataEvent> data) {
        ...
        System.out.println(String.format("%s %s %s %s with price %7.2f", event.eventTime(), event.type(), transaction.type(), transaction.name(), transaction.price()));
    }
}
